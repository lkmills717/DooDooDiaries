  ...
  def profile
    ...
    if params[:start_date] && params[:end_date]
      if params[:type] == "Movements"
        ...
        @movements = @movements.where("date(created_at) >= ? and date(created_at) <= ?",params[:start_date], params[:end_date])
        @meals = @meals.where("date(created_at) >= ? and date(created_at) <= ?",params[:start_date], params[:end_date])
      end
